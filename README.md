...
- **Show Similar Items** — a checkbox in the Selling tab, visible only while a piece of equipment sits in the sale slot. When you place gear to sell, a background Auction House search runs (no tab switching) for the same **slot + armor or weapon type** with the **same required level**, narrowed to items carrying the **same stats** as the item being sold (stat names, any value). Example: listing level-14 cloth boots with Intellect + Stamina shows other level-14 cloth boots with exactly Intellect and Stamina — not ones that also have Spirit. Matches are listed in the Selling tab's current-prices panel alongside Auctionator's usual name-based results.
...
- **Filter by Stat** — the shopping tab's item dialog gains a *Filter by Stat* panel: checkboxes for the five primary stats (Strength, Agility, Stamina, Intellect, Spirit), a match-all/match-any condition, and a *Min DPS* box. Active filters drop non-matching results from the next search; *Reset All* clears the panel along with the rest of the dialog.
- **Saved Stat Filters** — adding a dialog search to a shopping list keeps its stat filter and condition with that list entry (account-wide, shared across characters). Searching the entry, or the whole list, reapplies each entry's own filter; editing the entry shows the saved filter in the panel, and saving the edit with the panel cleared removes it. Filters are keyed to the exact search text, so list export/import does not carry them.
...
- **Sale Scan** — a button right of the money display in the Selling tab, on the same line as the bottom-row buttons. One click runs a live exact-name price search for every distinct item in the bag panel, like scanning a shopping list (sequential, throttle-aware, first page only per item since results arrive cheapest-first and only the lowest price matters, with the current-prices panel showing the same loading spinner and "Search for item X/Y" progress line a list scan does, and the button flipping to *Cancel Scan* while it runs), feeding Auctionator's price database so *Rel. Value* and the green glow reflect current prices instead of your last scan. The scan steps aside as soon as you place an item for sale or run your own search.

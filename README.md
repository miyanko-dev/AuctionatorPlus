...
- **Decision tooltip** — while the Auction House is open, the price-history tooltip adds *Floor Range* (the band the daily minimum buyout moved in over the window) and *Scan Days* (how many days feed the average), so a trend percentage can be judged by the data behind it.
